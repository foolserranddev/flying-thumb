...
void saveSetup(){bool setupMode=WiFi.getMode()==WIFI_AP;if(!setupMode&&!requireAuth())return;if(!server.hasArg("plain")){server.send(400,"application/json","{\"error\":\"missing body\"}");return;}JsonDocument d;if(deserializeJson(d,server.arg("plain"))){server.send(400,"application/json","{\"error\":\"invalid JSON\"}");return;}String ssid=d["ssid"]|"",pass=d["password"]|"",name=d["name"]|deviceName,key=d["key"]|managementKey;ssid.trim();name.trim();key.trim();if(setupMode&&!ssid.length()){server.send(400,"application/json","{\"error\":\"Wi-Fi name required\"}");return;}if(!name.length())name=deviceId;deviceName=name.substring(0,31);managementKey=key.substring(0,63);saveDevice();if(ssid.length())saveNetwork(ssid,pass);server.send(200,"application/json",ssid.length()?"{\"status\":\"network-saved\"}":"{\"status\":\"settings-saved\"}");scheduleRestart();}
