...
constexpr char FIRMWARE_VERSION_BASE[]="2.4.1";
...
bool startWpsAttempt(){esp_wifi_wps_disable();esp_wps_config_t config=WPS_CONFIG_INIT_DEFAULT(WPS_TYPE_PBC);esp_err_t enableResult=esp_wifi_wps_enable(&config);esp_err_t startResult=enableResult==ESP_OK?esp_wifi_wps_start(0):enableResult;if(enableResult==ESP_OK&&startResult==ESP_OK){wpsRetryPending=false;displayMessage("WPS PAIRING","Press router","WPS button");Serial.println("WPS pairing attempt started");return true;}Serial.printf("WPS start failed: enable=0x%x (%s), start=0x%x (%s)\n",enableResult,esp_err_to_name(enableResult),startResult,esp_err_to_name(startResult));wpsRetryPending=true;wpsRetryAt=millis()+WPS_RETRY_DELAY_MS;displayMessage("WPS STARTING","Radio retry...","");return false;}
void handleWpsState(){uint8_t event=pendingWpsEvent;if(event){pendingWpsEvent=0;if(event==1){esp_wifi_wps_disable();wpsRetryPending=false;rememberWpsNetwork();displayMessage("WPS SUCCESS","Connecting...","");delay(10);WiFi.begin();return;}esp_wifi_wps_disable();if(event==4){Serial.println("WPS PBC overlap: more than one router is advertising WPS");displayMessage("WPS OVERLAP","Only one router","can use WPS");wpsRetryPending=true;wpsRetryAt=millis()+5000;}else{Serial.printf("WPS attempt ended: event=%u reason=%u; retrying\n",event,pendingWpsFailReason);wpsRetryPending=true;wpsRetryAt=millis()+WPS_RETRY_DELAY_MS;displayMessage("WPS PAIRING","Press router","WPS button");}}if(wpsRetryPending&&(int32_t)(millis()-wpsRetryAt)>=0){if(millis()-wpsStartedAt>=WPS_PAIRING_WINDOW_MS){wpsRetryPending=false;displayMessage("WPS TIMED OUT","Short press","to retry");Serial.println("WPS pairing window expired");}else startWpsAttempt();}}
...
void beginWpsPairing(){pendingWpsEvent=0;wpsRetryPending=false;wpsStartedAt=millis();if(setupDnsActive){dns.stop();setupDnsActive=false;}MDNS.end();discovery.stop();esp_wifi_wps_disable();WiFi.mode(WIFI_MODE_NULL);delay(150);WiFi.mode(WIFI_STA);delay(250);esp_wifi_set_ps(WIFI_PS_NONE);startWpsAttempt();}
void initNetworkAndServer(){deviceId=makeDeviceId();prefs.begin("device",true);deviceName=prefs.getString("name",deviceId);managementKey=prefs.getString("key","");prefs.end();prefs.begin("network",true);savedSsid=prefs.getString("ssid","");savedPassword=prefs.getString("password","");wpsProvisioned=prefs.getBool("wps",false);prefs.end();WiFi.onEvent(onWifiEvent);if(!savedSsid.length()&&!wpsProvisioned){startSetupAp();startDiscovery();}else{WiFi.mode(WIFI_STA);WiFi.setHostname(makeHostName().c_str());if(wpsProvisioned)WiFi.begin();else WiFi.begin(savedSsid.c_str(),savedPassword.c_str());displayMessage(deviceName.c_str(),"Connecting...","");uint32_t start=millis();while(WiFi.status()!=WL_CONNECTED&&millis()-start<STA_CONNECT_TIMEOUT_MS)delay(100);if(WiFi.status()==WL_CONNECTED)showConnected();else displayMessage("WIFI OFFLINE","Short: WPS","Hold: reset");}startServer();}
void handleNetworkAndServer(){handleWpsState();if(setupDnsActive)dns.processNextRequest();server.handleClient();handleDiscovery();if(fileBatchActive&&millis()-fileBatchTouchedAt>120000){fileBatchActive=false;finishUsbFileUpdate();}if(restartPending&&millis()>=restartAt)ESP.restart();}
