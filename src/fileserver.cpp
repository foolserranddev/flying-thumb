...
void initNetworkAndServer(){deviceId=makeDeviceId();logMemory("Network startup");prefs.begin("device",true);deviceName=prefs.getString("name",deviceId);managementKey=prefs.getString("key","");prefs.end();prefs.begin("network",true);savedSsid=prefs.getString("ssid","");savedPassword=prefs.getString("password","");wpsProvisioned=prefs.getBool("wps",false);bool pairRequested=prefs.getBool("pair",false);prefs.end();WiFi.onEvent(onWifiEvent);if(pairRequested){prefs.begin("network",false);prefs.remove("pair");prefs.end();wpsOnlyBoot=true;pendingWpsEvent=0;wpsRetryPending=false;wpsStartedAt=millis();displayMessage("WPS ACTIVE","Press router","WPS button");delay(50);releaseDisplayMemoryForWps();logMemory("WPS display released");WiFi.mode(WIFI_MODE_NULL);delay(150);WiFi.mode(WIFI_STA);delay(250);esp_wifi_set_ps(WIFI_PS_NONE);startWpsAttempt();return;}if(!savedSsid.length()&&!wpsProvisioned){startSetupAp();startDiscovery();}else{WiFi.mode(WIFI_STA);WiFi.setHostname(makeHostName().c_str());if(wpsProvisioned)WiFi.begin();else WiFi.begin(savedSsid.c_str(),savedPassword.c_str());displayMessage(deviceName.c_str(),"Connecting...","");uint32_t start=millis();while(WiFi.status()!=WL_CONNECTED&&millis()-start<STA_CONNECT_TIMEOUT_MS)delay(100);if(WiFi.status()==WL_CONNECTED)showConnected();else displayMessage("WIFI OFFLINE","Short: WPS","Hold: reset");}startServer();}
